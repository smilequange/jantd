 ...
 */
package cn.jantd.core.poi.excel.imports;
...
import cn.jantd.core.poi.excel.entity.params.ExcelCollectionParams;
import cn.jantd.core.poi.excel.entity.params.ExcelImportEntity;
import cn.jantd.core.poi.excel.entity.result.ExcelImportResult;
import cn.jantd.core.poi.excel.entity.result.ExcelVerifyHanlderResult;
import cn.jantd.core.poi.excel.imports.base.ImportBaseService;
import cn.jantd.core.poi.excel.imports.verifys.VerifyHandlerServer;
import cn.jantd.core.poi.exception.excel.ExcelImportException;
import cn.jantd.core.poi.exception.excel.enums.ExcelImportEnum;
import cn.jantd.core.poi.util.ExcelUtil;
...
public class ExcelImportServer extends ImportBaseService {
    ...
    private String getSaveUrl(ExcelImportEntity excelImportEntity, Object object) throws Exception {
        String url = "";
        if (CoreConstant.UPLOAD.equals(excelImportEntity.getSaveUrl())) {
    ...
    public ExcelImportResult importExcelByIs(InputStream inputstream, Class<?> pojoClass, ImportParams params) throws Exception {
        ...
        for (int i = 0; i < params.getSheetNum(); i++) {
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(" start to read excel by is ,startTime is {}", System.console());
            }
            if (isXSSFWorkbook) {
