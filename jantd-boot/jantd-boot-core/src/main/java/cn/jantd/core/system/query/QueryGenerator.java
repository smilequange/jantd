package cn.jantd.core.system.query;
...
public class QueryGenerator {
    ...
    private static Date getDateQueryByRule(String value, QueryRuleEnum rule) throws ParseException {
        Date date = null;
        if (value.length() == CoreConstant.NUMBER_TEN) {
    ...
    private static String getFieldConditionValue(Object value, boolean isString) {
        ...
        if (str.startsWith(CoreConstant.EXCLAMATION_MARK)) {
            str = str.substring(1);
        } else if (str.startsWith(CoreConstant.GREATER_THAN_OR_EQUAL)) {
            str = str.substring(2);
        } else if (str.startsWith(CoreConstant.LESS_THAN_OR_EQUAL)) {
            str = str.substring(2);
        } else if (str.startsWith(CoreConstant.GREATER_THAN)) {
            str = str.substring(1);
        } else if (str.startsWith(CoreConstant.LESS_THAN)) {
            str = str.substring(1);
        }
        if (isString) {
            return " '" + str + "' ";
    ...
    private static String getLikeConditionValue(Object value) {
        String str = value.toString().trim();
        if (str.startsWith(CoreConstant.STAR) && str.endsWith(CoreConstant.STAR)) {
            return "'%" + str.substring(1, str.length() - 1) + "%'";
        } else if (str.startsWith(CoreConstant.STAR)) {
            return "'%" + str.substring(1) + "'";
        } else if (str.endsWith(CoreConstant.STAR)) {
