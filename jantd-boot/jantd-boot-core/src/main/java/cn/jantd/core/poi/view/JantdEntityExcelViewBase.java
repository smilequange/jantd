...
@Controller(NormalExcelConstants.JANTD_ENTITY_EXCEL_VIEW)
public class JantdEntityExcelViewBase extends BaseMiniAbstractExcelView {

	public JantdEntityExcelViewBase() {
		super();
	}
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (model.containsKey(NormalExcelConstants.MAP_LIST)) {
			...
			for (int i = 1; i < list.size(); i++) {
				new ExcelExportServerExcelExport().createSheet(workbook, (ExportParams) list.get(i).get(NormalExcelConstants.PARAMS), (Class<?>) list.get(i).get(NormalExcelConstants.CLASS), (Collection<?>) list.get(i).get(NormalExcelConstants.DATA_LIST));
			}
		} else {
