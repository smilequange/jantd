...
import cn.jantd.modules.system.model.SysPermissionTree;
...
import cn.jantd.modules.system.util.PermissionDataUtil;
...
public class SysPermissionController {
    ...
    private JSONObject getPermissionJsonObject(SysPermission permission) {
        JSONObject json = new JSONObject();
        // 类型(0：一级菜单 1：子菜单 2：按钮)
        if (SystemConstant.NUMBER_TWO.equals(permission.getMenuType())) {
            return null;
    ...
    private boolean isWWWHttpUrl(String url) {
        return url != null && (url.startsWith(SystemConstant.HTTP_PREFIX) || url.startsWith(SystemConstant.HTTPS_PREFIX) || url.startsWith("{{"));
    }
